...
Cubby 不是浏览器版 VS Code，也不是多人协作 IDE。代码写入、重构、测试命令和 Git 操作优先交给 agent session 在终端内完成；Cubby 本身优先提供远程访问安全、会话控制、终端连续性、运行预览、Git diff、文件查看和环境诊断。
...
- Agent 工作流闭环：Git changes、file explorer、port previews、runtime diagnostics。
